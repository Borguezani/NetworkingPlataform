...
export async function POST(request: Request, { params }: { params: Params }) {
    try {
        ...
        if (parsed.decision === 'aprovado') {
            ...
            await prisma.application.update({ where: { id }, data: { status: ApplicationStatus.aprovado, processedAt: new Date(), processedBy: header ?? 'admin' } })
        ...
        // reject
        await prisma.application.update({ where: { id }, data: { status: ApplicationStatus.rejeitado, processedAt: new Date(), processedBy: header ?? 'admin' } })
